...
export class DictDetailsComponent implements OnInit {
    ...
    validate() {
        ...
        let fetchState = this.dictState(this.dict.dictSchema).filter(x => {
            if (x.sourceCount > 1) {
                return x.sourceCount > 1;
            }
            else if (x.source === '' || x.target === '') {
                return x;
            }
        });
        ...
        if (fetchState.length == 0) {
            ...
            this.toast.setMessage('Some rows needs your attention before vaildation.', 'danger');
        }
    };
}
